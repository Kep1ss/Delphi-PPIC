...
uses
  ...
  FireDAC.Comp.DataSet, FireDAC.Comp.Client, Vcl.Menus, tmsAdvGridExcel;

type
  TFGanttChart = class(TForm)
    ...
    procedure sg1ClipboardBeforePasteCell(Sender: TObject; ACol, ARow: Integer;
...
procedure TFGanttChart.AdvSmoothButton1Click(Sender: TObject);
var
  i,x         : integer;
  qty_order   : integer;
...
procedure TFGanttChart.AdvSmoothButton3Click(Sender: TObject);
begin
  FDaftarSOGant.ShowMOdal;
end;

procedure TFGanttChart.btnAddClick(Sender: TObject);
begin
...
procedure TFGanttChart.btnGenerateTableClick(Sender: TObject);
var
  i , x , j: integer;
  MinTgl : TDate;
  MaxTgl : TDate;
...
function TFGanttChart.convertDate(AStr: String): String;
var
  dd    : String;
  mm    : String;
  yy    : String;
...
procedure TFGanttChart.CreateChart;
...
procedure TFGanttChart.CreateGrid;
begin
  with sg1 do begin
     ClearAll;
...
procedure TFGanttChart.D1Click(Sender: TObject);
begin
  IO2.XLSExport(ExtractFilePath(Application.ExeName)+'Format Import GC '+ENoSo.Text+' Container '+EnoContainer.Text+'.xls');
end;

procedure TFGanttChart.GenerateDate(ARow: integer;ADate:TDate);
var
  Jamkerja          : Integer;
...
function TFGanttChart.GetStarDate(ADate: TDate;id_divisi:String;KapasitasDivisi:Integer;var KapasitasTerpakai:Integer): TDate;
begin
...
procedure TFGanttChart.I1Click(Sender: TObject);
var
  Asg : TAdvStringGrid;
  i , x , j   : integer;
  LamaHari : Integer;
...
begin
  ...
  if OpenDialog1.Execute then begin
     ...
     try
       IO.AdvStringGrid := Asg;
       IO.XLSImport(OpenDialog1.FileName);

       sg1.BeginUpdate;
       for i := 2 to Asg.RowCount -1 do begin
         if (Asg.Cells[4,i] <> '') and (asg.Cells[12,i] <>'') then begin

            sg1.Dates[11,i-1] := StrToDate(Asg.Cells[12,i]);
            sg1.Dates[13,i-1] := StrToDate(Asg.Cells[14,i]);

            LamaHari := DaysBetween(sg1.Dates[11,i-1],sg1.Dates[13,i-1])+1;
            ManHour  := StrToIntDef(nilai(sg1.Cells[6,i-1]),0);
            JamKerja := StrToIntDef(nilai(sg1.Cells[7,i-1]),0);

            sg1.Cells[9,i-1]  := IntToStr(LamaHari);

            JmlKaryawan := Ceil((ManHour / (JamKerja * 60)) / LamaHari);
            sg1.Cells[8,i-1]  := IntToStr(JmlKaryawan);

            AlokasiWaktu  := JmlKaryawan * JamKerja * 60;

            StartDate := sg1.Dates[11,i-1];
            for x := 0 to LamaHari do begin

               SQ1.Close;
               SQ1.SQL.Text := 'INSERT INTO detail_jadwal ('+
               'id_divisi,tgl,sono,item_id,'+
               'qty_order,total_menit,gc_code,kapasitas,manhour,no_urut,alokasi_tng_kerja,jam_kerja) '+
               'VALUES (:id_divisi,:tgl,:sono,:item_id,:qty_order,'+
               ':total_menit,:gc_code,:kapasitas,:manhour,:no_urut,:alokasi_tng_kerja,:jam_kerja) ';
               SQ1.ParamByName('id_divisi').AsString       := sg1.Cells[14,i-1];
               SQ1.ParamByName('tgl').AsDate               := StartDate + x;
               SQ1.ParamByName('sono').AsString            := ENoSo.Text;
               SQ1.ParamByName('item_id').AsString         := sg1.Cells[16,i-1];
               SQ1.ParamByName('qty_order').AsString       := sg1.Cells[17,i-1];

               if ManHour  >= AlokasiWaktu then begin
                  SQ1.ParamByName('total_menit').AsInteger    := AlokasiWaktu;
                  ManHour := ManHour - AlokasiWaktu;
               end else if AlokasiWaktu > ManHour then begin
                  SQ1.ParamByName('total_menit').AsInteger    := ROUND(ManHour / JmlKaryawan);
               end;

               SQ1.ParamByName('gc_code').AsString            := Ecode.Text;
               SQ1.ParamByName('kapasitas').AsInteger         := StrToIntDef(sg1.Cells[15,i-1],0) * StrToIntDef(sg1.Cells[7,i-1],0) * 60 ;
               SQ1.ParamByName('manhour').AsInteger           := StrToIntDef(nilai(sg1.Cells[6,i-1]),0);
               SQ1.ParamByName('no_urut').AsInteger           := StrToIntDef(sg1.Cells[18,i-1],0);
               SQ1.ParamByName('alokasi_tng_kerja').AsString  := StrToDecimal(sg1.Cells[8,i-1]);
               SQ1.ParamByName('jam_kerja').AsString          := StrToDecimal(sg1.Cells[7,i-1]);
               SQ1.ExecSQL;
          ...
          SQ4.Open(' SELECT MIN(tgl) as Startdate , MAX(tgl) as Enddate FROM '+
           ' detail_jadwal WHERE id_divisi="'+sg1.Cells[14,i-1]+'" AND item_id="'+sg1.Cells[16,i-1]+'" '+
           ' AND gc_code="'+Ecode.Text+'"');
           ...
           for x := 0 to RangeTanggal -1 do begin
              ...
              if SQ4.FieldByName('total_menit').AsInteger > 0 then begin
                ...
                ExQry(Q4,'SELECT initial_color FROM divisi WHERE id="'+sg1.Cells[14,i-1]+'"',True);
                sg1.Cells[19+GapDate,i-1]    := SQ4.FieldByName('total_menit').AsString;
                sg1.Colors[19+GapDate,i-1]   := GetValInt(Q4,'initial_color');
                sg1.ColorsTo[19+GapDate,i-1] := GetValInt(Q4,'initial_color');
                sg1.FontColors[19+GapDate,i-1] := clWhite;
                sg1.FontSizes[19+GapDate,i - 1] := 8;
              end;
              SQ4.Next;
...
function TFGanttChart.IdDivisi: String;
var
  i : integer;
  id_divisi : String;
...
procedure TFGanttChart.KosongkanGrid;
var
  c , i : integer;
begin
...
procedure TFGanttChart.ResetChart;
begin
  //ResetColor(1);
  sg1.ColCount := 18;
end;

procedure TFGanttChart.ResetColor(Arow:Integer;Acol:integer);
...
procedure TFGanttChart.sg1CanEditCell(Sender: TObject; ARow, ACol: Integer;
  var CanEdit: Boolean);
begin
  CanEdit := ACOl in [5,7,8,10,11,12,13];
end;

procedure TFGanttChart.sg1ClipboardBeforePasteCell(Sender: TObject; ACol,
...
procedure TFGanttChart.sg1DatePickerCloseUp(Sender: TObject; ARow,
  ACol: Integer);
  var
  KapasitasDivisi : Integer;
  KapasitasTerpakai : integer;
...
procedure TFGanttChart.sg1GetAlignment(Sender: TObject; ARow, ACol: Integer;
  var HAlign: TAlignment; var VAlign: TVAlignment);
begin
  if ARow = 0 then
     HAlign := taCenter;
...
procedure TFGanttChart.sg1GetEditorType(Sender: TObject; ACol, ARow: Integer;
  var AEditor: TEditorType);
begin
...
function TFGanttChart.FindDivisi(id_divisi:integer):integer;
var
  i : integer;
begin
  for i := 2 to FGanttChart.sg1.RowCount -1 do begin
     if StrToIntDef(FGanttChart.sg1.Cells[14,i],0) = id_divisi then
     begin
       Result := i ;
       Break;
     end;
...
function TFGanttChart.FindStartRow: Integer;
var
  i : integer;
begin
...
procedure TFGanttChart.sg1KeyPress(Sender: TObject; var Key: Char);
var
  i , x , j: integer;
  MinTgl : TDate;
  MaxTgl : TDate;
...
procedure TFGanttChart.sg1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
...
procedure TFGanttChart.TampilAwal;
 var
  _Bulk : String;
begin
  ClearText(Self);
...
function TFGanttChart.toDate(AStr: String): TDate;
var
  dd    : String;
  mm    : String;
  yy    : String;
...
procedure TFGanttChart.UpdateDate( ADate: TDate);
var
  Jamkerja          : Integer;
  JumlahKaryawan    : Integer;
  ManHour           : Integer;
...
procedure TFGanttChart.UpdateKapasitas(Id_divisi:String;JmlPekerja:Integer;Arow:integer);
var
  i : integer;
begin
  for i := 1 to sg1.RowCount -1 do begin
